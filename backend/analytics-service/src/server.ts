...
export function createServer(customLogger?: winston.Logger) {
  ...
  const connectDB = async () => {
    const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/fleet-analytics';
  ...
  app.get('/api/debug', (_req: Request, res: Response) => {
    ...
    res.status(200).json({
      status: 'OK',
      message: 'Analytics Service debug route',
      timestamp: new Date().toISOString(),
  ...
  app.get('/debug', (_req: Request, res: Response) => {
    ...
    res.status(200).json({
      status: 'OK',
      message: 'Analytics Service ROOT debug route',
      timestamp: new Date().toISOString(),
      service: 'analytics-service-root'
    });
  });
  ...
  app.use('/api', setupAnalyticsRoutes(logger));
  ...
  const startServer = async (): Promise<Server> => {
    ...
    try {
      ...
      logger.info('Service will continue without Kafka connectivity. Will retry connections as needed.');
    }
    
    // Start the server
    return app.listen(PORT, () => {
      logger.info(`⚡️ Analytics Service running on port ${PORT}`);
    });
  };
...
if (require.main === module) {
  const { startServer, cleanup } = createServer();
